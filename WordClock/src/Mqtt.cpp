...
void Mqtt::setup() {
  _pubSubClient.setServer(ClockConfig.mqttHost, ClockConfig.mqttPort);
  _pubSubClient.setCallback([this] (char* topic, byte* payload, unsigned int length) { this->callback(topic, payload, length); });
}
...
void Mqtt::publishBrightness(uint8_t brightness) {
  if (ClockConfig.mqttEnabled == true && _pubSubClient.connected() == true) {
    char payload[4];
    ...
    _pubSubClient.publish(MQTT_LIGHT_BRIGHTNESS_STATE_TOPIC, payload, true);
...
void Mqtt::publishColor(uint32_t color) {
  if (ClockConfig.mqttEnabled == true && _pubSubClient.connected() == true) {
    ...
    _pubSubClient.publish(MQTT_LIGHT_RGB_STATE_TOPIC, payload, true);
    Serial.println("Published");
  }
}
...
void Mqtt::publishLightState() {
    _pubSubClient.publish(MQTT_LIGHT_STATE_TOPIC, "on", true);
}
...
void Mqtt::reconnect() {
  ...
  // Attempt to connect
  if (_pubSubClient.connect("wordclock", ClockConfig.mqttUsername, ClockConfig.mqttPasswd)) {
    Serial.println("INFO: connected");
    // Once connected, publish an announcement...
    this->publishLightState();
    // ... and resubscribe
    _pubSubClient.subscribe(MQTT_LIGHT_RGB_COMMAND_TOPIC);
    _pubSubClient.subscribe(MQTT_LIGHT_BRIGHTNESS_COMMAND_TOPIC);
  } else {
